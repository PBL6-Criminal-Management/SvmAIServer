...
def detect_face():
    ...
    if not globalVariables.isTrain:
        if download_model('model.joblib', globalVariables.Model_folder_id):
    ...
    if isPredictable:
        return jsonify({'isPredictable': isPredictable, 'label': int(result['label']), 'confidence': result['confidence'], 'image': image}), 200
    else:
